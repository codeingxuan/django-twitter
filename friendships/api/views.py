...
class FriendshipViewSet(viewsets.GenericViewSet):
    ...
    def followers(self, request, pk):
        # GET /api/friendships/1/followers/?page=<page_num>
    ...
    def list(self, request):
        if 'to_user' in request.query_params:
            to_user = request.query_params['to_user']
            friendships = Friendship.objects.filter(to_user_id=to_user).order_by('-created_at')
        ...
        if 'from_user' in request.query_params:
            from_user = request.query_params['from_user']
            friendships = Friendship.objects.filter(from_user_id=from_user).order_by('-created_at')
